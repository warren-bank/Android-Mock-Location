...
public class SharedPrefsState {
    ...
    public short diff(SharedPrefsState that) {
        ...
        if (!is_equal(that.trip_origin_lat, trip_origin_lat))                            diff_fields |= (1 << 5);  //  32
        if (!is_equal(that.trip_origin_lon, trip_origin_lon))                            diff_fields |= (1 << 6);  //  64
        if (!is_equal(that.trip_destination_lat, trip_destination_lat))                  diff_fields |= (1 << 7);  // 128
        if (!is_equal(that.trip_destination_lon, trip_destination_lon))                  diff_fields |= (1 << 8);  // 256
        if (that.trip_duration != trip_duration)                                         diff_fields |= (1 << 9);  // 512
